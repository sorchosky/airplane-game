...
- 2026-09-25 · art · BotW-inspired palette locked in `src/styles/tokens.ts`/`.css`; Sheikah-cyan `accent` reserved for interactive UI, not the plane livery (cream body, red-orange stripe) · keeps the one saturated color meaningful instead of competing with terrain hues
- 2026-09-25 · art · Type: `Cinzel` (display) + `Inter` (body/HUD), both Google Fonts (SIL OFL 1.1), loaded via the Fonts API CSS in `tokens.css` · thin-serif display matches the BotW brief; Inter is proven for TV-distance legibility; self-hosting can follow later if offline load time becomes an issue
- 2026-09-25 · art · UI buttons are a `surface-hud` panel with a 2px `accent` border, not a solid `accent` fill · matches the Sheikah Slate look and avoids needing a separate "text on accent" token
