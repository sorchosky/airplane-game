...
/** Component-wise `expDamp` over a Vector3. Returns a new Vector3, leaves the inputs untouched. */
export function dampVector3(current: Vector3, target: Vector3, rate: number, dt: number): Vector3 {
  const t = 1 - Math.exp(-rate * dt)
...
export function chaseCameraOrientation(
  ...
  const forward = lookAtPosition.clone().sub(cameraPosition)
  if (forward.lengthSq() < 1e-9) forward.set(0, 0, -1)
  forward.normalize()

  const up = new Vector3(0, 1, 0).applyAxisAngle(forward, bank * rollFraction)
  const matrix = new Matrix4().lookAt(cameraPosition, lookAtPosition, up)
